...
const getMyChats = TryCatch(async (req, res, next) => {
  ...
  const transformedChats = chats.map(({ _id, name, members, groupChat }) => {
    ...
    return {
      ...
      //collect member Id only excluding me
...
const addMembers = TryCatch(async (req, res, next) => {
  ...
  if (!members || members.length < 1)
    return next(new ErrorHandler("please provide members"));
  ...
  if (chat.creator.toString() !== req.user.toString())
    return next(new ErrorHandler("forbidden Request", 404));

  const allNewMembersPromise = members.map((i) => User.findById(i, "name"));
  //we didnt use await cuz to avoid finding user one by one but fetched all at once
...
const removeMembers = TryCatch(async (req, res, next) => {
  ...
  if (chat.creator.toString() !== req.user.toString())
    return next(new ErrorHandler("forbidden Request", 403));
...
const sendAttachments = TryCatch(async (req, res, next) => {
  ...
  if (files.length < 1)
    return next(new ErrorHandler("Files are not provided", 404));
...
const deleteChat = TryCatch(async (req, res, next) => {
  ...
  if (chat.groupChat && chat.creator.toString() !== req.user.toString())
    return next(
      new ErrorHandler("you are not a allowed to delete this group", 403)
    );
  ...
  //here we havae to delete the all messages as well as attachments or files from cloudinary
